...
test("editorial review manifest selects 89 unique, traceable articles", async () => {
  const [manifestText, postsText] = await Promise.all([
  ...
  assert.equal(manifest.posts.length, 89);
  assert.equal(new Set(slugs).size, slugs.length);
  assert.deepEqual(topicCounts, { popular: 32, weekend: 38, festival: 23, water: 15, indoor: 12, family: 22 });
