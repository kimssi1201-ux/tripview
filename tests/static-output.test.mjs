...
test("editorial review manifest selects 92 unique, traceable articles", async () => {
  const [manifestText, postsText] = await Promise.all([
  ...
  assert.equal(manifest.posts.length, 92);
  assert.equal(new Set(slugs).size, slugs.length);
  assert.deepEqual(topicCounts, { popular: 35, weekend: 41, festival: 26, water: 15, indoor: 12, family: 25 });
