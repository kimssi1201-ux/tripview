...
test("editorial review manifest selects 88 unique, traceable articles", async () => {
  const [manifestText, postsText] = await Promise.all([
  ...
  assert.equal(manifest.posts.length, 88);
  assert.equal(new Set(slugs).size, slugs.length);
  assert.deepEqual(topicCounts, { popular: 31, weekend: 37, festival: 22, water: 15, indoor: 11, family: 21 });
