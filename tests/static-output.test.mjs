...
test("editorial review manifest selects 98 unique, traceable articles", async () => {
  const [manifestText, postsText] = await Promise.all([
  ...
  assert.equal(manifest.posts.length, 98);
  assert.equal(new Set(slugs).size, slugs.length);
  assert.deepEqual(topicCounts, { popular: 41, weekend: 47, festival: 32, water: 15, indoor: 12, family: 26 });
