...
test("editorial review manifest selects 100 unique, traceable articles", async () => {
  const [manifestText, postsText] = await Promise.all([
  ...
  assert.equal(manifest.posts.length, 100);
  assert.equal(new Set(slugs).size, slugs.length);
  assert.deepEqual(topicCounts, { popular: 43, weekend: 49, festival: 34, water: 15, indoor: 12, family: 28 });
