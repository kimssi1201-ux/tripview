...
test("editorial review manifest selects 69 unique, traceable articles", async () => {
  const [manifestText, postsText] = await Promise.all([
  ...
  assert.equal(manifest.posts.length, 69);
  assert.equal(new Set(slugs).size, slugs.length);
  assert.deepEqual(topicCounts, { popular: 12, weekend: 18, festival: 16, water: 12, indoor: 10, family: 8 });
