...
test("editorial review manifest selects 85 unique, traceable articles", async () => {
  const [manifestText, postsText] = await Promise.all([
  ...
  assert.equal(manifest.posts.length, 85);
  assert.equal(new Set(slugs).size, slugs.length);
  assert.deepEqual(topicCounts, { popular: 28, weekend: 34, festival: 19, water: 15, indoor: 11, family: 18 });
