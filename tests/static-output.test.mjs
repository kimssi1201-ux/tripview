...
test("editorial review manifest selects 87 unique, traceable articles", async () => {
  const [manifestText, postsText] = await Promise.all([
  ...
  assert.equal(manifest.posts.length, 87);
  assert.equal(new Set(slugs).size, slugs.length);
  assert.deepEqual(topicCounts, { popular: 30, weekend: 36, festival: 21, water: 15, indoor: 11, family: 20 });
