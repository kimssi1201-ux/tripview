...
test("editorial review manifest selects 101 unique, traceable articles", async () => {
  const [manifestText, postsText] = await Promise.all([
  ...
  assert.equal(manifest.posts.length, 101);
  assert.equal(new Set(slugs).size, slugs.length);
  assert.deepEqual(topicCounts, { popular: 44, weekend: 50, festival: 34, water: 15, indoor: 12, family: 29 });
