...
test("editorial review manifest selects 109 unique, traceable articles", async () => {
  const [manifestText, postsText] = await Promise.all([
  ...
  assert.equal(manifest.posts.length, 109);
  assert.equal(new Set(slugs).size, slugs.length);
  assert.deepEqual(topicCounts, { popular: 52, weekend: 58, festival: 41, water: 15, indoor: 12, family: 37 });
