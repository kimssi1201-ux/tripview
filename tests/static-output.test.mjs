...
test("editorial review manifest selects 72 unique, traceable articles", async () => {
  const [manifestText, postsText] = await Promise.all([
  ...
  assert.equal(manifest.posts.length, 72);
  assert.equal(new Set(slugs).size, slugs.length);
  assert.deepEqual(topicCounts, { popular: 15, weekend: 21, festival: 16, water: 12, indoor: 10, family: 10 });
