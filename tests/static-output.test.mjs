...
test("editorial review manifest selects 99 unique, traceable articles", async () => {
  const [manifestText, postsText] = await Promise.all([
  ...
  assert.equal(manifest.posts.length, 99);
  assert.equal(new Set(slugs).size, slugs.length);
  assert.deepEqual(topicCounts, { popular: 42, weekend: 48, festival: 33, water: 15, indoor: 12, family: 27 });
