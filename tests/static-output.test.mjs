...
test("editorial review manifest selects 76 unique, traceable articles", async () => {
  const [manifestText, postsText] = await Promise.all([
  ...
  assert.equal(manifest.posts.length, 76);
  assert.equal(new Set(slugs).size, slugs.length);
  assert.deepEqual(topicCounts, { popular: 19, weekend: 25, festival: 16, water: 13, indoor: 10, family: 12 });
