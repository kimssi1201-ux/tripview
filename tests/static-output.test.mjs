...
test("editorial review manifest selects 86 unique, traceable articles", async () => {
  const [manifestText, postsText] = await Promise.all([
  ...
  assert.equal(manifest.posts.length, 86);
  assert.equal(new Set(slugs).size, slugs.length);
  assert.deepEqual(topicCounts, { popular: 29, weekend: 35, festival: 20, water: 15, indoor: 11, family: 19 });
