...
test("editorial review manifest selects 75 unique, traceable articles", async () => {
  const [manifestText, postsText] = await Promise.all([
  ...
  assert.equal(manifest.posts.length, 75);
  assert.equal(new Set(slugs).size, slugs.length);
  assert.deepEqual(topicCounts, { popular: 18, weekend: 24, festival: 16, water: 12, indoor: 10, family: 12 });
