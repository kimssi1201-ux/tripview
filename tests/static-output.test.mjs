...
test("editorial review manifest selects 95 unique, traceable articles", async () => {
  const [manifestText, postsText] = await Promise.all([
  ...
  assert.equal(manifest.posts.length, 95);
  assert.equal(new Set(slugs).size, slugs.length);
  assert.deepEqual(topicCounts, { popular: 38, weekend: 44, festival: 29, water: 15, indoor: 12, family: 25 });
