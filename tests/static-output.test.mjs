...
test("editorial review manifest selects 73 unique, traceable articles", async () => {
  const [manifestText, postsText] = await Promise.all([
  ...
  assert.equal(manifest.posts.length, 73);
  assert.equal(new Set(slugs).size, slugs.length);
  assert.deepEqual(topicCounts, { popular: 16, weekend: 22, festival: 16, water: 12, indoor: 10, family: 10 });
