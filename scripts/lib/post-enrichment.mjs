...
function factsSection(facts) {
  ...
  const first = rows.length
    ? `현재 확인되는 안내는 ${rows.map(([label, value]) => `${label} ${value}`).join(" · ")}입니다.`
    : `${facts.name}의 고정 운영시간과 휴무일은 별도로 확인되지 않습니다. 날짜를 정했다면 출발 전에 운영 여부와 입장 가능 시간을 직접 확인하는 과정이 필요합니다.`;
  const second = rows.length
    ...
    : `정보가 비어 있다는 이유로 상시 운영한다고 판단하면 안 됩니다. 지도에 표시된 전화번호나 공식 채널에서 운영 여부를 확인하고, 연락이 되지 않으면 가까운 대체 코스를 함께 준비하는 편이 안전합니다.`;
  return ["확인된 운영 정보", [first, second]];
}
  ...
  return [
    ["주소와 도착 동선", [
      facts.address && !PLACEHOLDER_PATTERN.test(facts.address)
        ...
        : `${subject} 정확한 입구와 주차 지점이 별도로 확인되지 않습니다. 장소명만 입력해 출발하지 말고 지도에서 도착 지점과 도보 진입로를 대조한 뒤 이동하세요.`,
...
function buildFaq(facts) {
  ...
  return [
    ["운영시간은 어떻게 확인하나요?", answer(facts.hours, "고정 운영시간이 별도로 확인되지 않아 방문 날짜의 운영 여부와 입장 마감 시간을 직접 확인해야 합니다.")],
    ["쉬는 날이 있나요?", answer(facts.closed, "정기 휴무 정보가 별도로 확인되지 않았습니다. 임시 휴무가 있을 수 있으므로 출발 당일 다시 확인하세요.")],
    ["주차할 수 있나요?", answer(facts.parking, "주차 가능 여부가 별도로 확인되지 않았습니다. 장소 주변 공영주차장과 도보 동선을 함께 확인하세요.")],
...
function bookingAndCostSection(facts) {
  return ["비용과 예약 확인", [
    ...
    `온라인 예약을 이용한다면 날짜와 인원, 이용 항목, 취소 가능 시점을 결제 전에 다시 보세요. 현장 안내가 예약 화면과 다를 때를 대비해 예약 번호와 결제 내역을 저장하고, 환불이나 일정 변경이 필요한 경우 이용 전에 문의하는 편이 좋습니다.`,
  ]];
}
