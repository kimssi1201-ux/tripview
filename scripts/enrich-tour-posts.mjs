...
function buildTravelSections(post) {
  ...
  return [
    ...
    ['계절별 방문 팁', [
      `${season} 여행은 시간대 선택이 만족도를 크게 좌우합니다. 오전에는 이동과 관람을 먼저 하고, 한낮에는 실내 휴식이나 식사 시간을 넣은 뒤, 늦은 오후에 산책이나 사진 촬영을 붙이면 무리한 이동을 줄일 수 있습니다.`,
      `${title}을 중심으로 하루 일정을 짠다면 주변 목적지를 너무 많이 넣기보다 가까운 식사 장소, 카페, 짧은 산책 구간 정도만 연결하는 편이 좋습니다. 특히 주말에는 예상보다 주차와 대기 시간이 길어질 수 있으니 돌아오는 시간을 넉넉히 남겨두세요.`,
      `비가 예보되어 있거나 더위가 남아 있는 날에는 현장에서 오래 버티는 계획보다 대체 코스를 하나 준비하는 편이 현실적입니다. 가까운 실내 공간, 짧게 쉬어갈 매장, 귀가하기 쉬운 정류장을 미리 봐두면 일정이 갑자기 바뀌어도 부담이 적습니다.`
    ]]
  ];
}
...
function buildFaq(post) {
  ...
  if (post.category === '공연/축제') {
    return [
      ['입장료가 있나요?', `요금 기준은 ${fee}입니다. 먹거리, 체험, 판매 부스 이용 비용은 별도로 발생할 수 있습니다.`],
...
- 제목은 네이버 피드형 노출을 고려해 한글 기준 35~55자 안팎으로 작성한다.
- 제목 앞부분 18~24자 안에 지역명, 장소명, 계절 키워드 중 최소 1개가 들어가게 한다.
- 제목은 “장소명, 독자가 궁금해할 질문”… 구체 동선·예약·주차 체크 형식을 우선한다.
