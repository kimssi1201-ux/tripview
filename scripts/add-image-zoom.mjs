...
const ZOOM_CSS = `.thumb{display:block;overflow:hidden;background:var(--soft)}.latest-primary .thumb{aspect-ratio:1.34/1}.side-card .thumb{aspect-ratio:1.25/1}.card .thumb{aspect-ratio:1.45/1}.thumb img{height:100%;aspect-ratio:inherit;transition:transform .42s ease,filter .28s ease}.latest-primary:hover .thumb img,.latest-primary:focus-visible .thumb img,.side-card:hover .thumb img,.side-card:focus-visible .thumb img,.card:hover .thumb img,.card:focus-visible .thumb img{transform:scale(1.04)}.latest-primary:active .thumb img,.side-card:active .thumb img,.card:active .thumb img,.latest-primary.is-opening .thumb img,.side-card.is-opening .thumb img,.card.is-opening .thumb img{transform:scale(1.12);filter:brightness(.92)}.latest-primary.is-opening,.side-card.is-opening,.card.is-opening{pointer-events:none}@media(prefers-reduced-motion:reduce){.thumb img{transition:none}.latest-primary:hover .thumb img,.latest-primary:focus-visible .thumb img,.side-card:hover .thumb img,.side-card:focus-visible .thumb img,.card:hover .thumb img,.card:focus-visible .thumb img,.latest-primary:active .thumb img,.side-card:active .thumb img,.card:active .thumb img,.latest-primary.is-opening .thumb img,.side-card.is-opening .thumb img,.card.is-opening .thumb img{transform:none;filter:none}}`;

const CLICK_SCRIPT = `<script id="post-card-transition">(() => { const reduce = window.matchMedia('(prefers-reduced-motion: reduce)').matches; const selector = 'a.latest-primary, a.side-card, a.card'; document.addEventListener('click', (event) => { const card = event.target.closest(selector); if (!card || !card.href || card.target || event.metaKey || event.ctrlKey || event.shiftKey || event.altKey || event.defaultPrevented) return; const url = new URL(card.href, window.location.href); if (url.origin !== window.location.origin) return; if (reduce) return; event.preventDefault(); card.classList.add('is-opening'); window.setTimeout(() => { window.location.href = card.href; }, 180); }, { capture: true }); })();</script>`;
