...
const BEACH_WEATHER_SCRIPT = '<script src="/assets/beach-weather.js?v=beach-weather-20260725" defer></script>';
const BEACH_WEATHER_OPTIONS = [
  [1, "\uC744\uC655\uB9AC"],
  [2, "\uC655\uC0B0"],
  [3, "\uD558\uB098\uAC1C"],
  [4, "\uBBFC\uBA38\uB8E8"],
...
const BEACH_WEATHER_CSS = `
      .beach-weather-controls{display:flex;align-items:center;gap:10px;margin:-4px 0 18px}.beach-weather-controls label{color:var(--muted);font-size:13px;font-weight:800}.beach-weather-controls select{min-width:150px;border:1px solid var(--line);border-radius:0;background:#fff;color:var(--ink);padding:9px 10px;font:inherit;font-weight:800}.beach-weather-controls button{border:1px solid var(--ink);background:var(--ink);color:#fff;padding:9px 12px;font:inherit;font-size:12px;font-weight:800;cursor:pointer}.beach-weather-controls button:disabled{opacity:.5;cursor:wait}.beach-weather-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:0;border-top:1px solid var(--line);border-left:1px solid var(--line);margin:0}.beach-weather-metric{min-width:0;padding:14px 12px;border-right:1px solid var(--line);border-bottom:1px solid var(--line)}.beach-weather-label{color:var(--muted);font-size:12px;font-weight:800}.beach-weather-value{margin:5px 0 0;font-size:20px;font-weight:900}.beach-weather-meta{display:block;margin-top:4px;color:var(--muted);font-size:11px}.beach-weather-status,.beach-weather-partial{margin:0;padding:15px 0;color:var(--muted);font-size:13px;border-top:1px solid var(--line);border-bottom:1px solid var(--line)}.beach-weather-status.is-error{color:#b42318}.beach-weather-tide{margin-top:18px}.beach-weather-subtitle{display:block;margin-bottom:8px;font-size:15px}.beach-weather-tide-list{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:0;margin:0;padding:0;border-top:1px solid var(--line);list-style:none}.beach-weather-tide-row{display:flex;align-items:center;gap:8px;padding:10px 8px;border-bottom:1px solid var(--line)}.beach-weather-tide-type{color:var(--muted);font-size:12px}.beach-weather-tide-time{font-size:14px}.beach-weather-tide-level{margin-left:auto;color:var(--muted);font-size:11px}.beach-weather-source{margin:14px 0 0;color:var(--muted);font-size:11px;line-height:1.55}.beach-weather-source a{text-decoration:underline}.beach-weather-section .section-more{display:none}@media(min-width:760px){.beach-weather-grid{grid-template-columns:repeat(3,minmax(0,1fr))}.beach-weather-tide-list{grid-template-columns:repeat(4,minmax(0,1fr))}}@media(max-width:430px){.beach-weather-controls{align-items:stretch;flex-wrap:wrap}.beach-weather-controls select{flex:1;min-width:0}.beach-weather-controls button{white-space:nowrap}}`.trim();
const BEACH_INFO_CSS = `.beach-info{margin-top:22px;padding-top:18px;border-top:1px solid var(--line)}.beach-info-image{width:100%;height:auto;max-height:280px;object-fit:cover;margin:0 0 14px;background:var(--soft)}.beach-info-grid{display:grid;grid-template-columns:96px minmax(0,1fr);margin:0;border-top:1px solid var(--line)}.beach-info-label,.beach-info-value{padding:9px 0;border-bottom:1px solid var(--line);font-size:12px}.beach-info-label{color:var(--muted);font-weight:800}.beach-info-value{margin:0;font-weight:800}.beach-info-link{display:inline-block;margin-top:12px;font-size:12px;font-weight:800;text-decoration:underline}`;
...
function beachWeatherSection() {
  return `<section class="news-section beach-weather-section" id="beach-weather" aria-labelledby="beach-weather-title" data-headline="\uD574\uC218\uC695\uC7A5 \uB0A0\uC528" data-beach-weather>
    <div class="section-headline">
      <div><span class="section-kicker">WEATHER</span><h2 id="beach-weather-title">\uD574\uC218\uC695\uC7A5 \uB0A0\uC528</h2></div>
    </div>
    <div class="beach-weather-controls">
      <label for="beach-weather-select">\uD574\uC218\uC695\uC7A5</label>
      <select id="beach-weather-select" data-beach-weather-select aria-label="\uD574\uC218\uC695\uC7A5 \uC120\uD0DD">
        ${BEACH_WEATHER_OPTIONS.map(([id, name]) => `<option value="${id}">${esc(name)}</option>`).join("")}
      </select>
      <button type="button" data-beach-weather-refresh>\uC0C8\uB85C\uACE0\uCE68</button>
    </div>
    <div data-beach-weather-output aria-live="polite"><p class="beach-weather-status">\uB0A0\uC528 \uC815\uBCF4\uB97C \uBD88\uB7EC\uC624\uB294 \uC911\uC785\uB2C8\uB2E4.</p></div>
    <p class="beach-weather-source">\uAE30\uC0C1\uCCAD \uB0A0\uC528\uC640 \uD574\uC591\uC218\uC0B0\uBD80 \uD574\uC218\uC695\uC7A5 \uC815\uBCF4 \uAE30\uC900\uC785\uB2C8\uB2E4. \uD604\uC7A5 \uC548\uB0B4\uC640 \uC218\uC601\uAE08\uC9C0 \uC5EC\uBD80\uB97C \uC6B0\uC120\uD574\uC8FC\uC138\uC694. <a href="https://www.data.go.kr/data/15102239/openapi.do" target="_blank" rel="noopener">\uB0A0\uC528 \uCD9C\uCC98</a> · <a href="https://www.data.go.kr/data/15058519/openapi.do" target="_blank" rel="noopener">\uD574\uC218\uC695\uC7A5 \uC815\uBCF4 \uCD9C\uCC98</a></p>
  </section>`;
}
...
function categoryNav(sections) {
  ...
  const primaryItems = [
    ...
    ["booking", "\uC608\uC57D \uC804 \uCCB4\uD06C"],
    ["flight-deals", "\uD56D\uACF5\uAD8C"],
  ].filter(([id]) => sectionById.has(id) || id === "beach-weather");
  return [
...
function html(posts, products = [], accommodations = [], tnaProducts = [], flights = []) {
  ...
  const sectionHtml = sections
    ...
    .join("\n")
    .replace(/(<section class="news-section check-section" id="booking")/, `${beachWeatherSection()}\n${mrtHtml}\n${coupangHtml}\n${coupangWidgetHtml}\n$1`);
