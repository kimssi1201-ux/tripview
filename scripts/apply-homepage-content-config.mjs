...
const DESKTOP_LAYOUT_CSS = `
      ...
      @media(min-width:900px){
        ...
        .editorial-hero-grid{grid-template-columns:minmax(0,1.92fr) minmax(330px,1fr)}.hero-main{aspect-ratio:16/9}.hero-main h2{font-size:44px}.hero-main p{font-size:15px}.hero-content{padding:38px}.hero-rail{grid-template-columns:repeat(2,minmax(0,1fr));grid-template-rows:repeat(2,minmax(0,1fr))}.hero-rail-card{aspect-ratio:16/9}.hero-rail-card.magazine-card strong{font-size:17px}.hero-rail-content{padding:18px}
        .category-top{grid-template-columns:minmax(0,1.8fr) minmax(310px,1fr);gap:24px}.news-lead .lead-thumb{aspect-ratio:16/10}.news-lead.magazine-card strong{font-size:28px}.category-picks{grid-template-columns:1fr;gap:0;border-top:1px solid var(--line)}.category-picks .pick-card{display:grid;grid-template-columns:116px minmax(0,1fr);gap:14px;align-items:center;padding:12px 0;border-bottom:1px solid var(--line)}.category-picks .pick-thumb{aspect-ratio:16/10}.category-picks .pick-card strong{margin:0;font-size:15px;-webkit-line-clamp:2}.category-list{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:26px 20px;margin-top:28px;padding-top:28px}.category-list .news-row{display:block;padding:0;border:0}.category-list .row-thumb{aspect-ratio:16/10}.category-list .news-row strong{margin-top:9px;font-size:18px}.category-list .news-row em{margin-top:7px}
      ...
      @media(max-width:899px){.site-header{position:sticky}.hero-main .hero-thumb,.hero-rail-card{aspect-ratio:16/9}}
...
function homeRegionCard(group) {
  const lead = group.posts.find((post) => !isFestival(post) && regionCardImageOf(post)) || group.posts.find((post) => regionCardImageOf(post));
  const image = regionCardImageOf(lead);
  if (!image) return "";
  const thumb = `<span class="story-thumb"><img src="${esc(image)}" alt="${esc(group.label)} 여행 허브 대표 글" loading="lazy"></span>`;
