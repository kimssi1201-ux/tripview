...
function bookingSearch() {
  return `<div class="booking-search" aria-label="예약 상품 검색">
    <form class="booking-search-card" data-booking-search="accommodation">
      <strong>숙소 검색</strong>
      <label>지역 <input name="keyword" type="search" value="제주" placeholder="제주, 서울, 부산" autocomplete="off"></label>
    ...
    <form class="booking-search-card" data-booking-search="tna">
      <strong>투어티켓 검색</strong>
      <label>키워드 <input name="keyword" type="search" value="제주 투어" placeholder="오사카 투어, 입장권" autocomplete="off"></label>
      <label>정렬
        <select name="sort">
...
<html lang="ko">
  ...
  <body>
    ...
    <script>
      ...
      document.querySelectorAll('[data-booking-search]').forEach((form) => {
        form.addEventListener('submit', async (event) => {
          ...
          try {
            ...
            if (!items.length) {
              setBookingStatus(payload.message || '검색 결과가 없습니다. 다른 키워드로 다시 검색해 보세요.');
              return;
            }
            items.forEach(appendBookingResult);
          } catch (error) {
            setBookingStatus(error.message || '검색 중 오류가 발생했습니다.');
          }
        });
      });
